...
Ajouter un argument `.names` à `across()` pour que les variables recodées soient stockées dans de nouvelles colonnes nommées avec le suffixe `"_rec"`.
...
On repart du code final de l'exercice 2.3, qui recodait une série de variables de `hdv2003` en valeurs `TRUE`/`FALSE` dans de nouvelles variables avec le suffixe "_rec".
...
Sans utiliser `rowwise()`, calculer le plus simplement possible une nouvelle variable `total` qui contient le nombre total de `TRUE` pour les variables `cinema_rec` et `sport_rec`.

```{r}
hdv2003 %>%
    mutate(total = cuisine_rec + sport_rec)
```

En utilisant `rowwise()` et `sum()`, recalculer la variable `total` pour qu'elle contienne le nombre total de `TRUE` pour les variables `bricol_rec`, `cinema_rec` et `sport_rec`.

```{r}
hdv2003 %>%
    rowwise() %>%
    mutate(total = sum(cuisine_rec, sport_rec, bricol_rec))
```

En utilisant `rowwise()`, `sum()` et `c_across()`, recalculer la variable `total` pour qu'elle contienne le nombre total de `TRUE` pour toutes les variables se terminant par "_rec".

```{r}
hdv2003 %>%
    rowwise() %>%
    mutate(total = sum(c_across(ends_with("_rec"))))
```
...
# Formule
starwars %>%
    rename_with(~ paste0("liste_", .), .cols = where(is.list))
# Compacte
starwars %>%
...
Réécrire ce code en utilisant une fonction anonyme, avec la syntaxe de votre chois (classique, formule ou compacte).
