...
Nous avons dans la partie précédente comment écrire nos propres fonctions. Pour pouvoir aller plus loin nous allons rappeler ou aborder ici de nouveaux types de données et éléments du langage qui permettent de programmer des actions un peu plus complexes.
...
R propose de nombreuses structures de données différentes, et les extensions peuvent en implémenter de nouvelles. On va en présenter ici les trois structures les plus utilisées : les vecteurs atomiques, les listes et les tableaux de données.

### Vecteurs atomiques

Les vecteurs atomiques sont des structures qui comportent plusieurs éléments qui doivent tous être du même type. Ils ont déjà été évoqués section \@ref{vecteurs}.
...
Les listes sont une généralisation des vecteurs : elles contiennent également plusieurs éléments, mais ceux-ci peuvent être de n'importe quel type, y compris des objets complexes. Une liste peut donc contenir elle-même des vecteurs des listes, des tablaux de données...
...
On voit déjà que la manière dont R affiche le contenu d'une liste dans la console diffère de celle des vecteurs. Pour les vecteurs les éléments sont affichés les uns après les autres, dans le cas d'une liste ils sont les uns en dessous des autres, et séparés par leur indice numérique entre une paire de crochets.
...
Dans ce cas la fonction `str` peut être très utile pour afficher de manière plus compacte la structure de la liste :
...
Si on utilise à la place les douvles crochets :
...
Une conséquence très importante est que `[[]]` ne peut sélectionner qu'un seul élément, on ne peut donc pas lui passer un vecteur, contrairement à `[]` :
...
Là aussi, si le nom comporte des caractères spéciaux on doit les encadrer avec des quotes inverses :
...
```{block, type="rmdimporant"}
Attention à ne pas utiliser les crochets doubles pour modifier plusieurs éléments d'une liste. Ceux-ci ne génèreront pas d'erreur mais risquent d'avoir un comportement inattendu.
```

Enfin, si on souhaite supprimer un ou plusieurs éléments d'une liste, ils suffit de leur attribuer la valeur `NULL` :
...
On les utilisera ainsi pour retourner plusieurs résultats différents depuis une fonction.
...
L'exemple fictif suivant récupère les noms de tous les fichiers CSV du répertoire courant et les importent tous en un coup dans une liste à l'aide de `purrr::map`. On pourra ensuite utiliser cette liste de tableaux de données pour leur appliquer des transformations ou les fusionner.
...
On a déjà abordé les tableaux de données en manipulant des *data frames* ou des *tibbles*. Les seconds sont une variante des premiers, les différences entre les deux ayant été présentées section \ref{tibbles}. On parlera donc ici indistinctement de tableaux de données. 
...
On a déjà vu plusieurs manières de sélectionner des éléments dans un tableau de données. En particulier, on a vu qu'on pouvait sélectionner une colonne via l'opérateur `$` :
...
- data.frame est une liste de vecteurs de même longueur et de même type
- sélection avec [[]]
- sélection avec [,]
- colonnes-liste, colonnes-df
...
Un autre type d'instructions de R permettent de répéter du code plusieurs fois, soit en fonction d'une conditions soit selon les éléments d'un vecteur. On verra en complément dans la section \ref{purrr} d'autres fonctions tirées de l'extension `purrr` qui elles permettent d'appliquer une fonction en itérant sur les éléments de plusieurs objets.
...
`while` prend en arguments une condition et exécute le bloc de code qui suit tant que la condition est vraie :
...
Peut être remplacée plus simplement par `x[which(x > seuil)]`.

Un cas particulier où une boucle peut être particulièrement lente est lorsqu'on l'utilise pour créer un nouveau vecteur ou data frame.

Dans l'exemple suivant, on utilise une boucle pour générér une liste de vecteurs de nombres aléatoires dont la longueur est stockée dans [x]. 
...
Avec cette syntaxe, à chaque ajout d'élément à `l` R procède à une copie de la totalité de `l`. Si le coût en termes de performance est négligeable avec seulement quelques éléments, il peut vite devenir très importants avec un nombre d'éléments élevé.
