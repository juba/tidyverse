...
Calculer le nombre de vols selon l'aéroport de destination et le mois, et fusionnez la table `airports` sur le résultat avec `left_join`. Stocker le résultat final dans un objet nommé `flights_dest`. 
...
Créez une carte interactive des résultats avec `ggmap` et le code suivant :

```{r echo=FALSE}
library(ggmap)
map <- get_map(location = "USA", source="osm", zoom=7)
ggmap(map) +
