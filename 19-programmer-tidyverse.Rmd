...
On a vu comment utiliser des colonnes passées en argument pour accéder à leur contenu. On peut aussi vouloir passer en argument des noms de colonnes qu'on souhaite créer, par exemple avec un `mutate()`.
...
5. Si on souhaite grouper ou appliquer une même transformation sur plusieurs colonnes, on utilise `across()` et l'opérateur *curly curly* :
...
6. Si on indique les noms de plusieurs colonnes sous la forme d'un vecteur de chaînes de caractères pour utilisation dans un `select()`, un `across()` ou une autre fonction acceptant la *tidy selection*, on utilise `all_of()` ou `any_of()` :
...
7. Si on souhaite créer une nouvelle colonne à partir du nom contenu dans un argument, on l'utilise sous la forme d'une chaîne de caractères, entouré d'une paire d'accolades et avec l'opérateur `:=` :
...
8. Si on souhaite passer des variables dans un graphique `ggplot2`, que ce soit dans `aes()` ou pour du faceting, on les encadre avec l'opérateur *curly curly* :
...
Modifier `my_table` pour qu'elle accepte trois arguments `df`, `var1` et `var2`, et qu'elle retourne le résultat du count appliqué à `by` et à `var`.
