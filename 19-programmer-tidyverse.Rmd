...
```{r echo=FALSE, ref.labe="progtidy-1-1-1"}
```
```{r}
...
```{r echo=FALSE, ref.labe="progtidy-1-1-2"}
```
...
```{r echo=FALSE, ref.labe="progtidy-1-1-3"}
```
...
```{r}
graph_points(starwars, height, mass, "red")
```
...
```{r}
graph_points(starwars, height, mass, sex)
```
...
graph_points <- function(df, var_x, var_y, color) {
    ...
    aesthetics <- aes(x = {{var_x}}, y = {{var_y}}, color = {{color}})
    ggplot(df) +
...
La fonction utilisant `[,]` évite d'avoir à utiliser l'opérateur *curly curly*, ainsi que l'ambiguïté liée à la présence d'un argument `height` et d'une colonne `height`.
</div>
\fi
