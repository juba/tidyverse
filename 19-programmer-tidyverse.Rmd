...
Dans certains cas de figure, on peut vouloir outrepasser ce *data masking*. Par exemple, dans le cas suivant, la nouvelle colonner `note_michelin` n'est pas créée à partir des nouvelles données de l'objet `note`, mais à partir de celles de la colonne `note`.
...
Certaines fonctions du *tidyverse* permettant de sélectionner des colonnes acceptent des expressions spécifiques

Une autre spécificité de certaines fonctions du *tidyverse* réside dans le mode de sélection des colonnes à base d'un "mini-langage" permettant des expressions comme :
...
Une difficulté survient quand les noms de colonnes du tableau ne sont pas saisis directement mais proviennent d'un argument de fonction.
...
restos %>% select_cols(c(evaluation, places))
```
...
De la même manière, si on utilise un argument de fonction pour sélectionner des variables avec `select()`, on doit l'entourer avec l'opérateur *curly curly*, et on peut dès lors utiliser toutes les possibilités de *tidy selection*.
...
5. Si on indique une spécification de plusieurs colonnes via la *tidy selection* pour utilisation dans un `select`, un `across` ou une autre fonction, on l'encadre de l'opérateur *curly curly* :
...
Créer une fonction `my_table` qui prend en arguments un tableau de données `df` et une variable `var` et qui renvoit le résultat de `count()` sur la variable `var` de `df`. 

Vérifier avec :

```{r echo=FALSE, ref.labell="progtidy-1-1-1"}
```
```{r}
...
Modifier `my_table` pour qu'elle accepte trois arguments `df`, `by` et `var`, et qu'elle retourne le résultat du count appliqué à `by` et à `var`.
...
Modifier `my_table` pour qu'elle accepte deux arguments `df` et `vars`, et qu'elle applique le `count` à toutes les variables indiquées dans `vars`.
...
Le code suivant ajoute une nouvelle colonne `diff_height` au tableau `starwars`, qui contient la différence entre la valeur de `height` et la moyenne de `height` selon les valeurs de `species`.
...
Créer une fonction `diff_mean_height` qui accepte un seul argument `by` et qui ajoute à `starwars` une colonne `diff_height` calculée de la même manière, mais en appliquant le `group_by` au `by` passé en paramètre.
...
Modifier la fonction pour que le nom de la colonne ajoutée ne soit pas `diff_mean` mais le nom de la variable `var` suivi d'un suffixe de votre choix.
...
Modifier à nouveau `diff_mean` pour pouvoir passer à `by` une sélection de variables.
...
Créer une fonction `graph_points` qui prend en arguments un tableau de données et deux variables, et raffiche le nuage de points de ces deux variables.

```{r echo=FALSE, ref.label='progtidy-2-2a'}
```
...
Ajouter un quatrième argument `color` à la fonction `graph_points()`, qui permet de spécifier la couleur des points (la même couleur pour tous les points).

```{r echo=FALSE, ref.label='progtidy-2-2b'}
```
...
```{r echo=FALSE, ref.label='progtidy-2-2c'}
```
...
*Facultatif :* modifier `graph_points()` pour que l'argument `color` puisse accepter aussi bien une variable qu'une couleur fixe. On pourra tester si la valeur passée à `color` est bien une colonne du tableau de données.

Indication : on pourra s'aider du résultat de la fonction suivante.
...
```{r echo=FALSE, ref.label='progtidy-2-2d'}
```
...
```{r}
graph_points(starwars, height, mass, color = "#F46789")
```
...
Créer une fonction `affiche_planete` qui affiche le message "Nom de la planète", lit la réponse de l'utilisateur, et affiche à l'aide d'un `fliter()` les lignes du tableau `starwars` pour lesquelles la variable `homeworld` correspond au nom saisi.
...
*Conseil :* pour récupérer un vecteur de colonnes à partir de la saisie de l'utilisateur, on pourra utiliser les fonctions `str_split()` et `str_trim()`.
...
Créer une fonction `filter_height` qui prend en argument trois paramètres `df`, `height_var` et `height`, et qui retourne les lignes de `df` pour laquelle la variable correspondant à `height_var` sont supérieures à `height`. Créer cette fonction sans utiliser `dplyr` mais avec l'opérateur `[,]`.

```{r echo=FALSE, ref.label='progtidy-4-1'}
```
```{r}
filter_height(starwars, "height", 200)
```
...
filter_height <- function(df, height_var, height) {
    df[df[[height_var]] > height, ]
}
```
</div>
\fi
...
filter_height <- function(df, height_var, height) {
    df[df[[height_var]] > height & !is.na(df[[height_var]]), ]
}
```
</div>
\fi
...
La fonction utilisant `[,]` évite d'avoir à utiliser l'opérateur *curly curly*, ainsi que l'ambiguïté liée à la présence simultanée d'un argument `height` et d'une colonne `height`.
</div>
\fi
