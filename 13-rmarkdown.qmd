...
::::{.callout-tip}
Si vous souhaitez apprendre et utiliser un système de génération et de publication de documents, il est désormais recommandé de passer à [Quarto](https://quarto.org), qui prend la suite de RMarkdown. Une grande partie des informations de ce chapitre restent quand même pertinentes puisque la structure et la syntaxe des documents Quarto sont assez similaires de celles des documents RMarkdown.
::::
...
On peut indiquer le titre, l'auteur du document ainsi que le format de sortie par défaut (il est possible de modifier facilement ces éléments par la suite). Plutôt qu'un document classique, on verra @sec-templates qu'on peut aussi choisir de créer une présentation sous forme de slides (entrée *Presentation*) ou de créer un document à partir d'un modèle (Entrée *From Template*).
...
## Modèles de documents {#sec-templates}
...
- [revealjs](https://github.com/rstudio/revealjs) pour des présentations HTML basées sur [reveal.js](http://lab.hakim.se/reveal-js/#/)
