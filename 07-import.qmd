...
Il est très vivement conseillé de travailler avec les projets de RStudio pour faciliter l'accès aux fichiers et pouvoir regrouper l'ensemble des éléments d'une analyse dans un dossier (voir @sec-projets).
...
L'extension `dplyr` est dédiée à la manipulation de données, elle est présentée @sec-dplyr. En installant l'extension complémentaire `dbplyr`, on peut utiliser `dplyr` directement sur une connection à une base de données générée par `DBI` :
