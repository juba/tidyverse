...
# Représentations graphiques avec `ggplot2` {#ggplot2}
...
Une des particularités de `ggplot2` est qu'elle part du principe que les données relatives à un graphique sont stockées dans une table (*data frame*, *tibble* ou autre).
...
Dans ce qui suit on utilisera le jeu de données issu du recensement de la population de 2012 inclus dans l'extension `questionr` (résultats partiels concernant les communes de plus de 2000 habitants de France métropolitaine). On charge ces données et on en extrait les données de 5 départements :
...
Un graphique `ggplot2` s'initialise à l'aide de la fonction `ggplot()`. Les données représentées graphiquement sont toujours issues d'un tableau de données (*data frame* ou *tibble*), qu'on peut passer en argument `data` à la fonction :
...
On peut modifier certains attributs graphiques d'un `geom` en lui passant des arguments supplémentaires. Par exemple, pour un nuage de points, on peut modifier la couleur des points avec l'argument `color`, leur taille avec l'argument `sier`, et leur transparence avec l'argument `alpha` :
...
**La règle est simple mais très importante :** 
...
```{r, message=FALSE}
ggplot(data = rp) + 
  geom_histogram(aes(x = cadres)) + facet_wrap(~departement)
```
...
Le premier argument est toujours le titre donné à la légende :
...
Représenter la répartition de la proportion des différentes tailles de commune pour les deux départements. Pour cela, faire un diagramme en bâtons (`geom_bar`) et utiliser l'argument `position = "fill"`.

```{r echo = FALSE}
ggplot(rp69) + geom_bar(aes(x = departement, fill = pop_cl), position = "fill")
