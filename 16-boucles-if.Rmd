...
Dans l'exemple suivant, on réécrit la fonction `summaries` vue précédemment pour prendre en compte le fait qu'un nom de variable non existante puisse être passé en argument. Si c'est le cas on passe directement à la variable suivante.
...
Autre exemple, si on souhaite remplacer dans tous les éléments d'un vecteur de chaînes de caractères le caractère "µ" par le caractère "o", on pourrait être tenter de faire une boucle du type :

```{r}
mots <- c("brµuette", "mµtµ", "iglµµ")
for (i in seq_along(mots)) {
    mots[i] <- str_replace_all(mots[i], "µ", "o")
}
```
