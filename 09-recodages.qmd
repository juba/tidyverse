...
À noter que dans ce cas, on peut utiliser l'opérateur `%in%`^[Pour accéder à la page de documentation de fonctions comme `%in%`, on ne peut pas utiliser `?%in%`, qui renvoie une erreur. Vous pouvez faire `?"%in%"`, `help("%in%")` ou, dans ce cas, `?match`, car les deux fonctions sont documentées sur la même page d'aide.], qui teste si une valeur fait partie des éléments d'un vecteur :
...
Les tests sont notamment utilisés par le verbe `filter` de `dplyr` (voir @sec-filter) qui permet de sélectionner certaines lignes d'un tableau de données. On peut ainsi sélectionner les individus ayant entre 20 et 40 ans en filtrant sur la variable `age` :
...
Pour plus d'informations sur les vecteurs et les structures de données de R, voir @sec-structures-donnees.
