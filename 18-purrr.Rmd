...
On peut dès lors utiliser `map_dfr()` et `read_csv()` pour lire tous ces fichiers en une seule fois et les concaténer avec `bind_rows()`.
...
L'intérêt principal de `modify()` est qu'elle propose deux variantes, `modify_if()` et `modify_at()`, qui sélectionnent les éléments soit via une fonction soit par leur nom ou leur position, et qui n'appliquent la fonction de transformation qu'aux éléments sélectionnés.
...
On peut aussi utiliser la notation compacte de type formule, mais dans ce cas il faut bien se souvenir que `.x` correspond à la valeur de l'élément courant, et `.y` à son nom.
