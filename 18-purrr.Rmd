...
Dans ce cas, on doit utiliser `map()` et conserver le résultat sous forme de liste. 
...
On peut évidemment enchaîner les `map()` si on veut effectuer des opérations supplémentaires.
...
Pour pouvoir calculer les corrélations souhaitées, on doit itérer sur les deux vecteurs `var1` et `var2` *en parallèle*, et calculer la corrélation entre la colonne correspondant à la valeur courante de `var1` et celle correspondant à la valeur courante de `var2`.
...
Calculer et retourner les mêmes résultats, mais cette fois sous la forme d'un unique tableau de données, avec une ligne par matière.
...
# Ou bien
parcours <- parcours %>%
    map(~ str_replace(.x, "Flixevile", "Flixeville"))
```
</div>
\fi
...
2. le résultat du test du Khi2 appliqué aux deux variables
...
*Facultatif :* filtrer la liste obtenue à l'étape précédente pour ne conserver que les éléments dont la p-value du test du Khi2 est inférieure à 0.001.
