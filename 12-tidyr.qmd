# Mettre en ordre avec `tidyr` {#sec-tidyr}

{{< include _setup.qmd >}}

## Tidy data

Comme indiqué @sec-tidydata, les extensions du *tidyverse* comme `dplyr` ou `ggplot2` partent du principe que les données sont "bien rangées" sous forme de *tidy data*.
...
d <- d %>%
  filter(
    country %in% c("France", "Germany", "Belgium", "Switzerland", "Spain", "Italy"),
    year >= 2002
  ) %>%
  select(country, year, pop) %>%
  pivot_wider(names_from = year, values_from = pop)
...
```{r warning=FALSE, paged.print=FALSE, echo = FALSE}
d <- d %>% bind_rows(
  list(country = "France", continent = "Europe", year = 2002, variable = "density", value = 94)
)
kable(d)
```
...
### `separate_rows` : séparer une colonne en plusieurs lignes {#sec-separate-rows}
