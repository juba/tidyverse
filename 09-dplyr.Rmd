...
Par ailleurs, les fonctions de `dlpyr` sont en général plus rapides que leur équivalent sous R de base, elles permettent donc de traiter des données de grande dimension.

`dplyr` part du principe que les données sont *tidy* (voir la section \@ref(tidy-data)). Les fonctions de l'extension peuvent s'appliquer à des tableaux de type `data.frame` ou `tibble`, et elles retournent systématiquement un `tibble` (voir la section \@ref(tibbles)).
...
`filter` sélectionne des lignes d'un tableau de données selon une condition. On lui passe en paramètre un test, et seules les lignes pour lesquelles ce test renvoit `TRUE` (vrai) sont conservées. Pour plus d'informations sur les tests et leur syntaxe, voir la section \@ref(tests).
...
Le jeu de données `nycflights13` est un exemple de jeu de données réparti en plusieurs tables. Ici on en a trois : les données sur les vols, celles sur les aéroports et celles sur les compagnies aériennes sont dans trois tables distinctes.
...
On remarquera que si des colonnes sont manquantes pour certaines tables, comme les colonnes `tz` et `dst` de `t3`, des `NA` sont automatiquement insérées. 
...
Très souvent, les données relatives à une analyse sont réparties dans plusieurs tables différentes. Dans notre exemple, on peut voir que la table `flights` contient seulement les codes des aéroports de départ et d'arrivée :
...
On crée ensuite une sous-table de vols en ne conservant que les colonnes `year`, `month` et `day`, et la clé `dest` qui nous permet de faire le lien avec la table `airports` :
...
Par exemple, si on souhaite fusionner la table `airlines` sur la table `flights`, on peut se contenter de faire :

```{r}
left_join(flights, airlines)
```

En effet, la seule colonne commune aux deux tables est `carrier`, et c'est bien la clé qui permet d'associer les lignes d'une table à celles de l'autre. On notera que `dplyr` affiche un message indiquant sur quelles colonnes la jointure s'est faite.
