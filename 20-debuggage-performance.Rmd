...
Trouver la cause d'un bug n'est pas toujours évident, mais heureusement il existe plusieurs méthodes et outils permettant de faciliter un peu les choses.
...
L'outil le plus simple, rudimentaire et parfois décrié mais qui reste efficace, est de rajouter un simple `print()` dans le code de notre fonction pour examiner le contenu d'un objet à un moment donné.
...
On teste notre fonction sur une variable du jeu de données `hdv2003` inclus dans R base. Tout semble fonctionner.
